...
class VolumeController():
    ...
    def level_audio(self, indata, outdata, frames, time, status):
        ...
        # Normalize the data coming in from the microphone to calculate the current level
            ...
            ml_num = s[0]*100
            scaled_ml = ((100-0)/(self.max-self.min)) * (ml_num - self.max) + 100
            ...
            if abs(int(scaled_value) - volume.actual) > abs(int(scaled_ml) - volume.actual):
                current_audio_level = int(scaled_ml)
            print("Device VOLUME: ", volume.actual)
            if current_audio_level - 5 < self.desired_audio_level:
                # Something sketchy happens where the audio comes in at 0
                if current_audio_level >= 8.0:
                    print("INCREASING VOLUME")
                    device.set_volume(volume.actual + 1)
            elif current_audio_level + 5 > self.desired_audio_level:
